<x-admin-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-slate-400">Update category details for {{ $category->name }}</p>
        </div>

        {{-- Form --}}
        <div class="max-w-2xl">
            <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="bg-slate-900/50 border border-white/10 rounded-xl shadow-xl overflow-hidden">
                @csrf
                @method('PUT')
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  class="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-400 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
