<x-admin-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-white mb-2">Edit Room</h1>
            <p class="text-slate-400">Update details for {{ $room->name }}</p>
        </div>

        {{-- Form --}}
        <div class="max-w-3xl">
            <form method="POST" action="{{ route('admin.products.update', $room) }}" class="bg-slate-900/50 border border-white/10 rounded-xl shadow-xl overflow-hidden">
                @csrf
                @method('PUT')
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="room_category_id"
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" class="bg-slate-900" {{ old('room_category_id', $room->room_category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  class="w-full px-4 py-3 bg-slate-800 border border-white/10 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none">{{ old('description', $room->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-400 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
